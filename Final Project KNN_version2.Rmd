...
```{r}
data<- read.csv("/Users/audreyhimes/Documents/UVA F21/DS 3001/DS-3001-new/DS3001FinalProject/College_Data.csv")  # reading in the data set 
# View(data)
...
```{r}
first<- ggplot(data, aes(x=StudentFacultyRatio, color=Private)) +
  geom_histogram(fill = "white", position="dodge", bins = 10) + ggtitle("Student to Faculty Ratios") + theme_bw()

sec<- ggplot(data, aes(x=FullTimeUndergrads, color=Private)) +
  geom_histogram(fill = "white", position="dodge", bins = 10) + ggtitle("Full Time Undergraduate Students")+ theme_bw()

third <- ggplot(data, aes(x=DonatingAlumniPercentage, color=Private)) +
  geom_histogram(fill = "white", position="dodge", bins = 10) + ggtitle("Percentages of Alumni who Donate")+ theme_bw()

fourth<- ggplot(data, aes(x=GraduationRate, color=Private)) +
  geom_histogram(fill = "white", position="dodge", bins = 10) + ggtitle("Graduation Rates") + theme_bw()

ggarrange(first, sec, third, fourth,
                    ncol = 2, nrow = 2)
```

```{r}
fifth<- ggplot(data, aes(x=NewStudentsEnrolled, color=Private)) +
  geom_histogram(fill = "white", position="dodge", bins = 10) + ggtitle("Numbers of New Students Enrolled") + theme_bw()

sixth <- ggplot(data, aes(x=Top10Percent, color=Private)) +
  geom_histogram(fill = "white", position="dodge", bins = 10) + ggtitle("Percent of Students in Top 10% of Class") + theme_bw()

seventh<- ggplot(data, aes(x=OutStateTuition, color=Private)) +
  geom_histogram(fill = "white", position="dodge", bins = 10) + ggtitle("Out of State Tuition") + theme_bw()

eighth<- ggplot(data, aes(x=AppsRecieved, color=Private)) +
  geom_histogram(fill = "white", position="dodge", bins = 10) + ggtitle("Number of Applications Received") + theme_bw()

ggarrange(fifth, sixth, seventh, eighth,
                    ncol = 2, nrow = 2)
```

#### Exploratory Variable Analysis Conclusions
First looking at the mean values table, the variables that appear to deviate the most between private and public schools are Applications Received, Percentage of Alumni who Donate, Number of Full-Time Undergraduate Students, Graduation Rate, Number of New-Students Enrolled, Out of State Tuition, Student-Faculty Ratio, and Percent of Students who are in the Top 10 Percent of their Class.  

To further investigate the deviations of these variables between public and private schools, we can examine their distributions. From the histograms, it appears that the Percentage of Alumni who Donate, Graduation Rates, Students in the Top 10 Percent of their Class, and Out of State Tuition are the variables that shift the most from public to private universities.  In all four cases, private schools have distributions centered around larger values.  This suggests that private schools may have a significantly larger percentage of donating alumni, graduation rate, percentage of students in the top 10% of their class, and tuition.  Given that these variables appear to differ the most between private and public schools, these variables may have the greatest importance in predicting whether a school is public or private.  
...
All of the variables identified in our initial exploratory data analysis that appeared to differ between private and public universities are important to the model.  Out of state tuition has an importance metric of 91.5, percentage of alumni who donate has a metric of 63.5, graduation rate has a metric of 48.2, and percentage of students in the top 10% of their class has a metric of 23.4.  These variables, along with the other variables of relatively high importance, contribute to a greater area under the ROC curve that compares sensitivity and specificity.  More area under the ROC curve indicates greater sensitivity and specificity measures, ultimately corresponding to a more accurate classifier model, such as our kNN model that classifies private vs public universities.  
